...
def chapter3(D, figs):
    ...
    D.p("หน้าเว็บ (Gradio) ออกแบบให้มีขั้นตอนเดียว: อัปโหลดหรือวางภาพ ระบบตรวจให้อัตโนมัติ แล้วแสดงการ์ด "
        "สรุปผล ภาพผลพร้อมกรอบและป้ายภาษาไทย ตารางรายการตำหนิเรียงตามระดับความเสี่ยง และส่วนขยายแสดงภาพ "
        "ผลของ Stage 1 โครงหน้าจอโดยรวมแสดงในภาพต่อไปนี้")
    D.figure(figs["wireframe"], "โครงหน้าจอ (Wireframe) ของหน้าเว็บแสดงผลการตรวจ", width=6.2)
    D.p("ตัวอย่างผลการตรวจจริงของระบบต้นแบบกับตำหนิแต่ละชนิด แสดงในภาพต่อไปนี้ โดยระบบวาดกรอบตำหนิ "
        "ป้ายชนิดภาษาไทย และค่าความมั่นใจกำกับ")
    D.figure("res_crazing.jpg", "ตัวอย่างผลการตรวจ — รอยแตกลายงา (crazing) บนภาพจากชุด benchmark", width=4.4)
    D.figure("res_scratches.jpg", "ตัวอย่างผลการตรวจ — รอยขีดข่วน (scratches)", width=4.4)
    D.figure("res_rust.jpg", "ตัวอย่างผลการตรวจ — สนิม (rust) บนภาพถ่ายเหล็กเส้นระดับสถานที่", width=4.6)
    D.p("ภาพต่อไปนี้เปรียบเทียบผลของแบบจำลองสองแบบบนภาพถ่ายจริงภาพเดียวกัน — แบบจำลองที่รายงานผลในเล่ม "
        ...
        "เชิงโดเมน")
    D.figure("res_thesis_model.jpg", "ผลบนภาพถ่ายจริง — แบบจำลองที่ฝึกบน grayscale (ไม่พบตำหนิ)", width=4.6)
    D.figure("res_adapted_model.jpg", "ผลบนภาพถ่ายจริง — แบบจำลองที่เพิ่มภาพถ่ายจริงในการฝึก (ตรวจพบสนิม)", width=4.6)
