...
├── app.py                   Prototype UI (Gradio) — 2 แท็บ: อัปโหลด / ถ่ายภาพ
...
- **2 แท็บรับภาพ:** อัปโหลด/วาง · ถ่ายจากกล้อง (แกลเลอรี `demo_samples/` 2 ภาพ/คลาส ขึ้นก่อน)
- **ตัวควบคุม 2 อัน:** เลือกโมเดล Stage 2 (`train-real3` แนะนำ / `train-real1` / `train-real2` / เล่มจบ `train-gray-n2`) ·
  โหมดความไว — `มาตรฐาน` / `ไว` (ลด threshold + TTA) / `ไวมาก` (+ ตรวจหลายสเกล/ตัดไทล์) ;
  ผลที่ผ่านเพราะลด threshold ขึ้นเป็น "อาจมี" เท่านั้น ไม่ขึ้น "ความเสี่ยงสูง"
